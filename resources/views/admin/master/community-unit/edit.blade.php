<x-admin>
     <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4"
        ...
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h3 class="fw-bolder mb-8">Edit Data RW</h3>
    ...
    <div class="card">
        <div class="card-body p-4">
            <form action="{{ route('dashboard.community-unit.update',$co->id) }}" method="post">
                @method('PUT')
                @csrf
                <div class="mb-4">
                <label for="exampleInputname" class="form-label">No Rw</label>
                <input type="text" nputmode="numeric" pattern="[0-9]*" value="{{ $co->no }}" id="inputAngka" oninput="this.value=this.value.replace(/[^0-9]/g,'')" name="no" class="form-control @error('no') is-invalid @enderror " id="exampleInputname" placeholder="Masukan No RW">
                   @error('no')
                ...
                <input type="text" name="leader_name" value="{{ $co->leader_name }}" class="form-control @error('leader_name') is-invalid @enderror " id="exampleInputname" placeholder="Masukan Nama Ketua RW">
                   @error('leader_name')
                    <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                    </span>
