<x-admin>
    ...
    <div class="card">
        <div class="card-body p-4">
            <form action="{{ route('resident.operator.update',$operator->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label for="exampleInputname" class="form-label">Nama Operator</label>
                    <input type="text" name="name" value="{{ $operator->name }}" class="form-control @error('name') is-invalid @enderror "
                        id="exampleInputname" placeholder="Masukan Nama Operator">
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" value="{{ $operator->email }}" class="form-control @error('email') is-invalid @enderror "
                        id="exampleInputname" placeholder="Masukan Nama Ketua RW">
                    @error('email')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
