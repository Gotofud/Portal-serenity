<x-admin>
    ...
    <div class="card">
        <div class="card-body p-4">
            <form action="{{ route('dashboard.block.update',$block->id) }}" method="post">
                @method('PUT')
                @csrf
                <div class="mb-4">
                <label for="exampleInputname" class="form-label">Nama BLok</label>
                <input type="text" name="name" value="{{ $block->name }}" class="form-control @error('no') is-invalid @enderror " id="exampleInputname" placeholder="Contoh : A1">
                   @error('name')
