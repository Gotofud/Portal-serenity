<x-admin>
    ...
    <x-partials.admin.form-modal :formRoute=" route('dashboard.vehicle-type.store')" id="addTipe"
        icon="ri ri-function-add-line" title="Tambah Data Jenis Kendaraan" subtitle="Wajib Diisi">
        @include('admin.master.vehicle-type._fields')
    </x-partials.admin.form-modal>
    <div class="card mt-5">
        <x-partials.admin.export_modal :exportExcel="route('dashboard.community-unit.export')"
            :exportPdf="route('dashboard.vehicle-type.exportPdf')" />
                ...
                <tbody class="table-border-bottom-0">
                    @foreach ($vehicle_type as $data)
                        ...
                        <x-partials.admin.form-modal id="editTipe{{ $data->id }}"
                            :formRoute="route('dashboard.vehicle-type.update', $data->id)" method="PUT"
                            title="Edit Jenis Kendaraan {{ $data->no }}">

                            @include('admin.master.vehicle-type._fields', ['data' => $data])
    ...
    <x-partials.admin.import_modal :downloadRoute="route('dashboard.template.download', 'vehicle-type')"
        :importRoute="route('dashboard.vehicle-type.import')" />
</x-admin>