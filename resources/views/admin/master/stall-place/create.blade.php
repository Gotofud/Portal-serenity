<x-admin>
    ...
    <div class="card">
        <div class="card-body p-4">
            <form action="{{ route('dashboard.stall-place.store') }}" method="post">
                @csrf
                <div class="mb-4">
                <label for="exampleInputname" class="form-label">Nama Kios</label>
                <input type="text" name="name" class="form-control @error('no') is-invalid @enderror " id="exampleInputname" placeholder="Contoh : A1">
                   @error('name')
                ...
                <label for="exampleInputname" class="form-label">No Rw</label>
                 <select class="form-select @error('community_id') is-invalid @enderror" id="rw" name="community_id" aria-label="Default select example">
                ...
                <label for="exampleInputname" class="form-label">No Rt</label>
                 <select class="form-select @error('neighborhood_id') is-invalid @enderror" id="rt" name="neighborhood_id" disabled>
                ...
                <input type="text" nputmode="numeric" pattern="[0-9]*"  id="inputAngka" oninput="this.value=this.value.replace(/[^0-9]/g,'')" name="stall_unit" class="form-control @error('stall_unit') is-invalid @enderror " id="exampleInputname" placeholder="Masukan Jumlah Unit">
                   @error('stall_unit')
                    <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                    </span>
                ...
                Rp        
                 ...
                 <input type="text" inputmode="numeric"  id="harga" name="rent_amount" class="form-control @error('rent_amount') is-invalid @enderror" placeholder="Masukan Harga Sewa" autocomplete="off"/>
                   @error('rent_amount')
                    <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                    </span>
