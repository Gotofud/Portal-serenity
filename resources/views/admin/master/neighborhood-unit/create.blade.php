<x-admin>
    ...
    <div class="card">
        <div class="card-body p-4">
            <form action="{{ route('dashboard.neighborhood-unit.store') }}" method="post">
                @csrf
                <div class="mb-4">
                <label for="exampleInputname" class="form-label">No RT</label>
                <input type="text" nputmode="numeric" pattern="[0-9]*" id="inputAngka" oninput="this.value=this.value.replace(/[^0-9]/g,'')" name="no" class="form-control @error('no') is-invalid @enderror " id="exampleInputname" placeholder="Masukan No RT">
                   @error('no')
