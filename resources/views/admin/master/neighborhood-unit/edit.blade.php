<x-admin>
     <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4"
        ...
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h3 class="fw-bolder mb-8">Edit Data RT</h3>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none"
                                    href="{{ route('dashboard.role.index') }}">Manajemen Rukun Tetangga</a>
                            </li>
    ...
    <div class="card">
        <div class="card-body p-4">
            <form action="{{ route('dashboard.neighborhood-unit.update',$nu->id) }}" method="post">
                @method('PUT')
                @csrf
                <div class="mb-4">
                <label for="exampleInputname" class="form-label">No RT</label>
                <input type="text" value="{{ $nu->no }}" nputmode="numeric" pattern="[0-9]*" id="inputAngka" oninput="this.value=this.value.replace(/[^0-9]/g,'')" name="no" class="form-control @error('no') is-invalid @enderror " id="exampleInputname" placeholder="Masukan No RT">
                   @error('no')
                ...
                <input type="text" name="leader_name" value="{{ $nu->leader_name }}" class="form-control @error('leader_name') is-invalid @enderror " id="exampleInputname" placeholder="Masukan Nama Ketua RW">
                   @error('leader_name')
                    <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                    </span>
